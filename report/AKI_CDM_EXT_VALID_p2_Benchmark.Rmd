...
for(pred_task in pred_task_lst){
  ...
  for(i in seq_along(yr_rg)){
    var_by_yr<-readRDS(paste0("./data/preproc/",pred_in_d,"d_var_by_yr_",pred_task,".rda"))[[i]]
...
*(I.a) Overall Prediction Performance*   
...
kable(perf_overall %>% arrange(pred_task,fs_type),
...
kable(perf_overall %>% arrange(pred_task,fs_type),
      caption="Table2 - 48-Hour Prediction of AKI1, AKI2, AKI3") %>%
  kable_styling("striped", full_width = F)
...
```{r final, include=F}
write.xlsx(final_out,file="./output/AKI_CDM_EXT_VALID_p2_Benchmark_TBL.xlsx")
