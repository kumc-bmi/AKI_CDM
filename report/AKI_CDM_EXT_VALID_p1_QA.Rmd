...
med_temp<-med_summ %>% 
  ...
  spread(summ,summ_val) %>%
  mutate(`1.encounters#`=suppressWarnings(ifelse(`1.encounters#` == "<11", 10,
...
med_top<-med_temp %>% 
  ...
  bind_rows(med_temp %>%
              arrange(desc(`1.encounters#`) %>%
                        slice(1:5))) %>%
  dplyr::mutate(hjust=c(0,0.2,-0.2,0,0,0,0,0.2,0.2,0.2),
                vjust=c(0,0,0,0,0,-0.5,-0.3,-0.1,0.1,0.3))
  

ggplot(med_temp,aes(x=dsa_grp,y=`3b.median_expos`,color=`1.encounters#`)) +
  geom_point() + scale_color_gradient2(high="red")+
  geom_text(data=med_top,aes(label=key,hjust=hjust,vjust=vjust))+
  labs(x="Start Date",y="Median Exposure (days)",color="Number of Encounters",
       title="Figure 5 - Medication Exposure Summaries")
```

Figure 5 demonstrates average exposures for drug starting at 1st, 2nd, 3rd,..., 7th and after 7th days since admission. It helps identify typical medciations dispensed (:01) or administered(:02) during the course of stay, as well as drugs of suspecious long exposures (e.g. 308395(vitamin C), 245723(vitamin A derivative)).
