...
vital_smoke_summ %<>% filter(enc_cnt!="<11") %>%
  arrange(key,desc(enc_cnt))
...
lab_temp<-lab_summ %>%
  ...
  mutate(label=ifelse(dense_rank(-enc_cnt)<=10 | dense_rank(-record_intensity)<=10,
                      key,""))
...
med_temp<-med_summ %>% 
  ...
  filter(!(summ=="enc_cnt" & summ_val=="<11")) %>%
  filter(!is.na(summ_val)) %>%
  mutate(summ_val=as.numeric(summ_val)) %>%
  spread(summ,summ_val) %>%
  filter(!is.na(enc_cnt)) %>%
  mutate(enc_cnt=ifelse(enc_cnt == "<11", 10,as.numeric(enc_cnt))) %>%
