...
params:
  DBMS_type:
    label: "DBMS Type (Type of SQL dialect):"
    value: Oracle
    input: select
    choices: [Oracle, tSQL, PostgreSQL]
---
...
conn<-connect_to_db(DBMS_type,config_file)
```
...
demo<-dbGetQuery(conn,
                 parse_sql(paste0("./inst/",DBMS_type,"/collect_demo.sql"),
                           cdm_db_schema=config_file$cdm_db_schema,
...
vital<-dbGetQuery(conn,
                  parse_sql(paste0("./inst/",DBMS_type,"/collect_vital.sql"),
                            cdm_db_schema=config_file$cdm_db_schema,
                            cdm_db_server=" ")$statement) %>%
...
kable(vital_smoke_summ %>% filter(`2.encounters#`!="<11"),
...
lab<-dbGetQuery(conn,
                parse_sql(paste0("./inst/",DMBS_type,"/collect_lab.sql"),
                          cdm_db_schema=config_file$cdm_db_schema,
                          cdm_db_server=" ")$statement) %>%
...
A total of `r nrow(lab_summ)` LOINC identifiable labs are eligible, among which `r nrow(lab_summ %>% filter(at_admission==1) %>% dplyr::select(key) %>% unique)` are collected at the day of admission, `r nrow(lab_summ %>% filter(within_3d==1) %>% dplyr::select(key) %>% unique)` within 3 days. Figure 1 shows the data density and intensity of labs concepts, which can help identify common labs (e.g. 2160-0 (SCr), 2823-3 (potassium), 2951-2(sodium), 17861-6(calcium), 3094-0(BUN), 1963-8(CO2), 33037-3 (Anion Gap), 2075-0(chloride), 718-7(hemoglobin),...), and labs with very high recording intensity (e.g. 2340-8(glucose)).
...
uhc_DRG<-dbGetQuery(conn,
                parse_sql(paste0("./inst/",DBMS_type,"/collect_DRG.sql"),
                          cdm_db_schema=config_file$cdm_db_schema,
                          cdm_db_server=" ")$statement) %>%
...
drg_temp<-DRG_summ %>% 
  ...
  mutate(`2.encounters#`=suppressWarnings(ifelse(`2.encounters#`=="<11",10,
...
dx<-dbGetQuery(conn,
               parse_sql(paste0("./inst/",DBMS_type,"/collect_dx.sql"),
                         cdm_db_schema=config_file$cdm_db_schema,
                         cdm_db_server=" ")$statement) %>%
...
px<-dbGetQuery(conn,
               parse_sql(paste0("./inst/",DBMS_type,"/collect_px.sql"),
                         cdm_db_schema=config_file$cdm_db_schema,
                         cdm_db_server=" ")$statement) %>%
...
med<-dbGetQuery(conn,
                parse_sql(paste0("./inst/",DBMS_type,"/collect_med.sql"),
                          cdm_db_schema=config_file$cdm_db_schema,
                          cdm_db_server=" ")$statement) %>%
...
med_temp<-med_summ %>% 
  ...
  spread(summ,summ_val) %>%
  filter(!is.na(`1.encounters#`)) %>%
  mutate(`1.encounters#`=ifelse(`1.encounters#` == "<11", 10,
                                as.numeric(`1.encounters#`))) %>%
...
med_top<-med_temp %>% 
  ...
  bind_rows(med_temp %>%
              ...
              slice(1:2) %>%
              ungroup) %>%
  arrange(dsa_grp,desc(`1.encounters#`)) %>%
  dplyr::mutate(hjust=rep(c(0.5,0.5,0.4,0.4),9),
                vjust=rep(c(-2,-1,0,1),9))

ggplot(med_temp,aes(x=dsa_grp,y=`1.encounters#`,color=`3b.median_expos`)) +
  geom_point() + 
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
  ...
  labs(x="Start Date",y="Number of Encounters",color="Median Exposure (days)",
       title="Figure 5 - Medication Exposure Summaries")
```

Figure 5 demonstrates average exposures for drug starting at 1st, 2nd, 3rd,..., 7th and after 7th days since admission. It helps identify typical medciations dispensed (:01) or administered(:02) during the course of stay (e.g. 1807627(sodium chloride), 1740467(ondansetron)), as well as drugs of relatively long exposures (e.g. 308395(ACE inhibitor), 799835(alvimopan)).
