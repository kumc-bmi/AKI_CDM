...
vital_smoke_summ<-vital %>%
  ...
  mutate(pat_cnt=ifelse(as.numeric(pat_cnt)<11 & as.numeric(pat_cnt)>0,"<11",as.character(pat_cnt)),
         ...
         enc_cnt=ifelse(as.numeric(enc_cnt)<11 & as.numeric(enc_cnt)>0,"<11",paste0(round(enc_prop,2)*100,"%"))) %>%
...
lab_temp<-lab_summ %>%
  ...
  mutate(label=ifelse(dense_rank(-`1.encounters#`)<=10 | dense_rank(-record_intensity)<=10,
                      key,""))

ggplot(lab_temp,aes(x=record_intensity,y=`1.encounters#`,label=label))+
...
drg_temp<-DRG_summ %>% 
  ...
  mutate(label=ifelse(dense_rank(-`2.encounters#`)<=10,
                      DRG,""))

ggplot(drg_temp,aes(x=`4c.mean_history`,y=`2.encounters#`,color=key1,label=label))+
  geom_point()+geom_text_repel()+
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
...
ggplot(dx_temp,aes(x=`4c.mean_history`,y=`2.encounters#`,label=label))+
  geom_point()+geom_text_repel()+
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
...
ggplot(px_temp,aes(x=`4c.mean_history`,y=`2.encounters#`,label=label))+
  geom_point()+geom_text_repel()+
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
...
```{r med_out, echo=F, warning=F, message=F, fig.height = 15, fig.width = 20}
med_temp<-med_summ %>% 
  ...
  mutate(label=ifelse(dense_rank(-`3b.median_expos`)<=3 | dense_rank(-`1.encounters#`)<=3,
                      key,""))

ggplot(med_temp,aes(x=dsa_grp,y=`1.encounters#`,color=`3b.median_expos`,label=label)) +
  geom_point() + geom_text_repel()+
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
