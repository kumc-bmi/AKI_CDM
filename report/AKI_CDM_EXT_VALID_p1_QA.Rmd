...
if(nrow(med_temp)>0){
  ...
  med_report<-med_temp %>%
    ...
    mutate(key2=trimws(key2),
           type=trimws(type)) %>%
    arrange(desc(enc_cnt)) %>%
    dplyr::select(key2,type) %>% 
    unique %>% dplyr::slice(1:3) %>%
    mutate(rx_name=lapply(key2, get_drug_nm, type))
  ...
  med_report<-med_temp %>%
    separate(key,c("rx_name","type","route"),":",extra="merge",fill="right") %>%
    mutate(rx_name=trimws(rx_name),
           type=trimws(type)) %>%
    arrange(desc(enc_cnt)) %>%
    dplyr::select(rx_name,type) %>% 
    mutate_at(vars(rx_name), get_drug_nm,.$type)
