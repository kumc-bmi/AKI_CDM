...
if(nrow(med_temp)>0){
  ...
  med_report<-med_temp %>%
    mutate(key=ifelse(params$incl_NDC,
                      gsub(".*_","",gsub("\\:.*","",key)),
                      gsub("\\:.*","",key))) %>%
    arrange(desc(enc_cnt)) %>%
    dplyr::select(key) %>% 
  ...
  description<-paste0("Figure4 demonstrates average exposures for drug starting at X days since admission. 
                      ...
                      (e.g. the typical medications identified are",freq_med,"; while drugs such as ",intens_med," 
...
A Total of `r length(unique(med_summ$key))` distinct `r ifelse(params$incl_NDC,"NDC","RXNORM")` medication concepts are discovered for the cohort and the overall medication (any) exposure for this cohort is `r paste0(round(med_density/enc_tot,2)*100,"%")`. `r print(description)`
