...
vital<-dbGetQuery(conn,
  ...
  dplyr::mutate(dsa=DAYS_SINCE_ADMIT) %>%
  dplyr::select(-MEASURE_DATE_TIME) %>%
...
lab<-dbGetQuery(conn,
                parse_sql(paste0("./inst/",params$DBMS_type,"/collect_lab.sql"),
                          ...
                          cdm_db_name=config_file$cdm_db_name,
                          cdm_db_schema=config_file$cdm_db_schema)$statement) %>%
  dplyr::select(PATID,ENCOUNTERID,LAB_LOINC,RESULT_NUM,RESULT_UNIT,SPECIMEN_DATE_TIME,DAYS_SINCE_ADMIT) %>%
  dplyr::mutate(dsa=DAYS_SINCE_ADMIT) %>%
...
lab_report<-lab_temp %>% arrange(desc(enc_cnt)) %>% slice(1:5) %>%
  bind_rows(
    lab_temp %>% arrange(desc(record_intensity)) %>% slice(1:3)
  ) %>%
  dplyr::select(key) %>%
...
[`r lab_report$key[8]`]: `r lab_report$link[8]`
...
## admission diagnosis
dxa<-dbGetQuery(conn,
               parse_sql(paste0("./inst/",params$DBMS_type,"/collect_dx_admit.sql"),
                         cdm_db_link=config_file$cdm_db_link,
                         cdm_db_name=config_file$cdm_db_name,
                         cdm_db_schema=config_file$cdm_db_schema)$statement) %>%
  ...
  dplyr::select(PATID,ENCOUNTERID,SERUM_CREAT_BASE,key,dsa) %>%
  unique
...
dxa_summ<-dxa %>%
  ...
  dplyr::summarize(record_cnt=n(),
                   ...
                   enc_cnt=length(unique(ENCOUNTERID)),
                   scr_mean=mean(SERUM_CREAT_BASE,na.rm=T),
                   scr_sd=sd(SERUM_CREAT_BASE,na.rm=T)) %>%
  ungroup %>%
  #HIPPA, low counts masking
  mutate(record_cnt=ifelse(as.numeric(record_cnt)<11,"<11",record_cnt),
         pat_cnt=ifelse(as.numeric(pat_cnt)<11,"<11",pat_cnt),
         enc_cnt=ifelse(as.numeric(enc_cnt)<11,"<11",enc_cnt)) %>%
  arrange(key)

#save
saveRDS(dxa,file="./data/AKI_dxa.rda")
# saveRDS(dxa_summ,file="./data/dxa_summ.rda")
...
dx_temp<-dx_summ %>% 
  ...
  mutate(enc_cnt=suppressWarnings(ifelse(enc_cnt=="<11",
                                         10,as.numeric(enc_cnt))),
...
dx_report<-dx_temp %>% 
  ...
  slice(1:6) %>%
  dplyr::select(key) %>%
  left_join(ccs_ref %>% filter(type=="dx"),by=c("key"="ccs_code")) %>%
  dplyr:select(key,ccs_name)


#admission diagnosis
dxa_temp<-dxa_summ %>% 
  dplyr::select(key,enc_cnt,scr_mean) %>% 
  mutate(enc_cnt=suppressWarnings(ifelse(enc_cnt=="<11",
                                         10,as.numeric(enc_cnt)))) %>%
  mutate(label=ifelse(dense_rank(-enc_cnt)<=10,key,""))

ggplot(dxa_temp,aes(x=scr_mean,y=enc_cnt,label=label))+
  geom_point()+geom_text_repel()+
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
  labs(title="Figure 3 - Admission Diagnoses vs. Baseline SCr")
...
dxa_report<-dxa_temp %>% 
  ...
  bind_rows(
    dxa_temp %>% 
      arrange(desc(scr_mean)) %>%
      slice(1:3)
  ) %>%
  dplyr::select(key) %>%
  mutate(pos=1:n()) %>% #in case sorting in left_join
...
There are `r length(unique(dxa_summ$key))` distcint CCS-group admission diagnoses for encounters of interest. Figure 3 layouts the ditribution of the admission diagnoses associated with patients' baseline serum creatinine characteristics (e.g. `r dx_report$key[1]`(`r dxa_report$ccs_name[1]`), `r dxa_report$key[2]`(`r dxa_report$ccs_name[2]`), `r dxa_report$key[3]`(`r dxa_report$ccs_name[3]`) are the most common diagnosis for this study cohort; while patients admitted due to `r dxa_report$key[4]`(`r dxa_report$ccs_name[4]`), `r dxa_report$key[5]`(`r dxa_report$ccs_name[5]`), `r dxa_report$key[6]`(`r dxa_report$ccs_name[6]` ) tends to have lower average baseline SCr. 
...
ggplot(px_temp,aes(x=dsa_grp,y=enc_cnt,label=label))+
  ...
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
  labs(title="Figure 3 - Procedure Density over Time")

px_report<-px_temp %>%
  top_n(n=5L,wt=enc_cnt) %>%
  dplyr::select(key) %>%
...
med<-dbGetQuery(conn,
  ...
  arrange(RX_START_DATE) %>%
  dplyr::mutate(rx_start_lead1=lead(RX_START_DATE),
                rx_end_lead1=lead(RX_END_DATE)) %>%
  replace_na(list(rx_start_lead1=RX_START_DATE,
                  rx_end_lead1=RX_END_DATE)) %>%
  ungroup %>%
  mutate(RX_START_DATE=pmin(RX_START_DATE,rx_start_lead1),
         RX_END_DATE=pmax(RX_END_DATE,rx_end_lead1)) %>%
  unique %>% 
  dplyr::select(-rx_start_lead1,-rx_end_lead1) %>%
...
med_report<-med_temp %>%
  arrange(desc(enc_cnt)) %>% slice(1:3) %>%
  bind_rows(
    med_temp %>%
      arrange(desc(median_expos)) %>% slice(1:3)
  ) %>%
  mutate(pos=1:n())%>%
  dplyr:select(key) %>%
  mutate(key=gsub("\\:.*","",key)) %>%
...
A Total of `r length(unique(med_summ$key))` distinct RXNORM medication concepts are discovered for the cohort. Figure 5 demonstrates average exposures for drug starting at 1st, 2nd, 3rd,..., 7th and after 7th days since admission. It helps identify typical medciations dispensed (:01) or administered(:02) during the course of stay. (e.g. the typical medications identified are `r med_report$key[1]`(`r med_report$rx_name[1]`), `r med_report$key[2]`(`r med_report$rx_name[2]`), `r med_report$key[3]`(`r med_report$rx_name[3]`); while `r med_report$key[1]`(`r med_report$rx_name[1]`), `r med_report$key[2]`(`r med_report$rx_name[2]`), `r med_report$key[3]`(`r med_report$rx_name[3]`) seems to be exposed for relatively long period on average).
