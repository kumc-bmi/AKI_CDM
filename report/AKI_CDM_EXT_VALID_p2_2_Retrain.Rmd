...
perf_metrics<-c("roauc","roauc_low","roauc_up",
               "prauc1","opt_sens","opt_spec",
               "opt_ppv","opt_npv")
...
```{r pred24_summ,echo=F,warning=F}
fs_type_opt<-c("no_fs","rm_scr_bun")
...
subgrp_age<-c()
for(i in seq_along(pred_task_lst)){
      valid<-readRDS(paste0("../data/model_ref/pred_in_",pred_in_d,"d_",fs_type,"_",pred_task_lst[i],".rda"))$valid %>%
      ...
      for(grp in seq_along(grp_vec)){
        ...
        subgrp_age %<>% 
          bind_rows(grp_summ %>% 
                      dplyr::mutate(grp_type="Subgrp_AGE",
                                    subgrp=grp_vec[grp],
  ...
  for(i in seq_along(pred_task_lst)){
      ...
      for(grp in seq_along(grp_vec)){
        ...
        subgrp_scr %<>% 
          bind_rows(grp_summ %>% 
                      dplyr::mutate(grp_type="Subgrp_Scr_Base",
                                    subgrp=grp_vec[grp],
                                    pred_task=paste0("stg",i,"up")))
      }
  }
...
perf_overall %<>%
  ...
  bind_rows(subgrp_scr %>% 
              dplyr::mutate(pred_in_d=pred_in_d,fs_type=fs_type)) %>%
  dplyr::select(pred_in_d,fs_type,grp_type,subgrp,pred_task,overall_meas,meas_val) %>%
...
row_grp_pos<-perf_overall %>% 
  arrange(grp_type,pred_task) %>%
  mutate(rn=1:n()) %>%
  dplyr::mutate(root_grp=gsub(":.*","",grp_type)) %>%
  group_by(root_grp,pred_in_d) %>%
  dplyr::summarize(begin=rn[1],
                   end=rn[n()]) %>%
  ungroup

kable(perf_overall %>% arrange(grp_type,pred_task),
...
```{r pred48_summ,echo=F,warning=F}
fs_type_opt<-c("no_fs","rm_scr_bun")
