...
For each prediction task, defined as "predict AKI stage X in Y days, with/without Scr", 1 intermedicate data, named as `..._valid_gbm_...rda`, should have been generated and saved in `./data/model_kumc/...`, which contains the predictted probabilities on the validataion set (the entire cohort at site).     
...
kable(perf_overall %>% arrange(grp,pred_task),
  ...
  group_rows("Overall-All_feature", row_grp_pos$begin[1],row_grp_pos$end[1]) %>%
  group_rows("Overall-Scr_and_BUN_removed", row_grp_pos$begin[2],row_grp_pos$end[2]) %>%
  group_rows("Subgroup-Age", row_grp_pos$begin[3],row_grp_pos$end[3]) %>%
  group_rows("Subgroup-Scr_Base", row_grp_pos$begin[4],row_grp_pos$end[4])
