...
params<-list(DBMS_type="tSQL",remote_CDM=FALSE)
...
# collect raw prescribing medication data
...
expos_all<-med$RX_EXPOS[order(med$RX_EXPOS)]

# report summaries for debugging
cat(nrow(med)," rows are collected.\n",
    length(unique(med$ENCOUNTERID))," distinct encounters are covered.\n",
    length(unique(med$key))," distinct rxnorm are identified.\n",
    "exposures quantiles are",paste(quantile(expos_all,probs=0:10/10),collapse = ","),".\n")
