...
from pcornet_cdm51_c008_r017.ENCOUNTER e
join pcornet_cdm51_c008_r017.DEMOGRAPHIC d
on e.PATID = d.PATID
where DATE_PART('day', e.DISCHARGE_DATE::timestamp - e.ADMIT_DATE::timestamp) >= 2 and -- LOS>=2
      ...
      e.ADMIT_DATE between Date '2010-01-01' and Date '2019-12-31'                   -- data collection window
)
select ENCOUNTERID
      ,PATID
      ,age_at_admit
...
select l.PATID
      ...
      ,l.RESULT_DATE
      ,l.RESULT_TIME
from pcornet_cdm51_c008_r017.LAB_RESULT_CM l
...
select PATID
      ...
      -- can't count distinct over partition in postgres, because this is only used for filtering
      -- rework with exists clause in subsequent query
...
group by scrb.PATID,scrb.ENCOUNTERID,init.ADMIT_DATE_TIME,scrb.LAB_ORDER_DATE,scrb.SPECIMEN_DATE_TIME, init.discharge_date_time,scrb.RESULT_DATE_TIME
;
...
from AKI_init aki
where exists (select 1 from pcornet_cdm51_c008_r017.DIAGNOSIS dx
...
select aki.ENCOUNTERID
from AKI_init aki
where exists (select 1 from pcornet_cdm51_c008_r017.PROCEDURES px
...
join pcornet_cdm51_c008_r017.PROCEDURES px
...
/*************************
 Consort Diagram
***************************/
DROP TABLE IF EXISTS gpc_aki_project.consort_diagram;
create table gpc_aki_project.consort_diagram as
