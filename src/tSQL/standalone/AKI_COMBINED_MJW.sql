/*******************************************************************************
 ...
 - PCORNET6.0_Jan2021 will be substituted by corresponding database name where CDM data is
 - pcornet will be substituted by corresponding CDM schema
********************************************************************************/
...
select e.ENCOUNTERID
      ...
      ,e.DRG_TYPE
      ,e.ADMITTING_SOURCE
from [PCORNET6.0_Jan2021].[pcornet].ENCOUNTER e
join [PCORNET6.0_Jan2021].[pcornet].DEMOGRAPHIC d
on e.PATID = d.PATID
where datediff(dd,e.ADMIT_DATE,e.DISCHARGE_DATE) >= 2 and
      e.ENC_TYPE in ('EI','IP','IS') and
      e.ADMIT_DATE between '2010-01-01' and '2019-12-31'
)
...
select l.PATID
      ...
      ,l.RESULT_DATE
      ,l.RESULT_TIME
from [PCORNET6.0_Jan2021].[pcornet].LAB_RESULT_CM l
...
where exists (select 1
			  from [PCORNET6.0_Jan2021].[pcornet].DIAGNOSIS dx
...
from #AKI_init aki
where exists (select 1
			  from [PCORNET6.0_Jan2021].[pcornet].[PROCEDURES] px
...
from #scr48 scr48
where exists (select 1 from [PCORNET6.0_Jan2021].[pcornet].[PROCEDURES] px
...
from #AKI_init aki
where exists (select 1 from [PCORNET6.0_Jan2021].[pcornet].DIAGNOSIS dx
...
join [PCORNET6.0_Jan2021].pcornet.[PROCEDURES] px
...
into #AKI_DEMO
from #AKI_onsets pat
left join [PCORNET6.0_Jan2021].[pcornet].DEMOGRAPHIC demo
on pat.PATID = demo.PATID
left join [PCORNET6.0_Jan2021].[pcornet].DEATH dth
...
into #AKI_VITAL
from #AKI_onsets pat
left join [PCORNET6.0_Jan2021].[pcornet].VITAL v
...
into #AKI_PX
from #AKI_onsets pat
left join [PCORNET6.0_Jan2021].[pcornet].[PROCEDURES] px
...
into #AKI_DX
from #AKI_onsets pat
join [PCORNET6.0_Jan2021].[pcornet].DIAGNOSIS dx
...
into #AKI_LAB
from #AKI_onsets pat
join [PCORNET6.0_Jan2021].[pcornet].LAB_RESULT_CM l
...
into #AKI_PMED
from #AKI_onsets pat
join [PCORNET6.0_Jan2021].[pcornet].PRESCRIBING p
...
into #AKI_DMED
from #AKI_onsets pat
join [PCORNET6.0_Jan2021].[pcornet].DISPENSING d
...
into #AKI_AMED
from #AKI_onsets pat
join [PCORNET6.0_Jan2021].[pcornet].MED_ADMIN m
