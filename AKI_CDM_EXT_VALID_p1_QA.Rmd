...
```{r, echo=F, warning=F, message=F}
kable(lab_summ %>% filter(freq_rk <=5), 
      caption="Table 3 - Lab Summaries (top)")
```

A total of `r nrow(lab_summ)` LOINC identifiable labs are eligible, among which `r nrow(lab_summ %>% filter(at_admission==1) %>% dplyr::select(key) %>% unique)` are collected at the day of admission, `r nrow(lab_summ %>% filter(within_3d==1) %>% dplyr::select(key) %>% unique)` within 3 days. The summaries `r unique(lab_summ$summ)[-(1:2)]` help identify odd lab values (e.g., 2160-0 (SCr) has some really strange trend in 3f.max in terms of days in hospital).
...
```{r, echo=F, warning=F, message=F}
kable(DRG_summ %>% arrange(desc(`1.patients#`)) %>% slice(1:20),
      caption="Table 4 - UHC DRG Summaries (top)")
...
```{r, echo=F, warning=F, message=F}
kable(dx_summ %>% dplyr::arrange(desc(`2.encounters#`)) %>% slice(1:20), 
      caption="Table 5 - Historical Diagnoses Summaries (top)")

dx_temp<-dx_summ %>% dplyr::select(key,`2.encounters#`,`4c.mean_history`) %>% 
  filter(`4c.mean_history`!= "<11") %>%
  mutate(`2.encounters#`=as.numeric(`2.encounters#`),
         `4c.mean_history`=as.numeric(`4c.mean_history`)) %>%
  dplyr::arrange(`2.encounters#`)

ggplot(dx_temp,aes(x=`4c.mean_history`,y=`2.encounters#`)) +
  geom_point() +
...
```{r, echo=F, warning=F, message=F}
kable(px_summ %>% arrange(desc(`2.encounters#`)) %>% slice(1:20),
      caption="Table 6 - Historical Procedures Summaries (top)")

px_temp<-px_summ %>% dplyr::select(key,`2.encounters#`,`4c.mean_history`) %>% 
  filter(`4c.mean_history`!= "<11") %>%
  mutate(`2.encounters#`=as.numeric(`2.encounters#`),
         `4c.mean_history`=as.numeric(`4c.mean_history`)) %>%
  arrange(`2.encounters#`)

ggplot(px_temp,aes(x=`4c.mean_history`,y=`2.encounters#`)) +
  geom_point() +
...
Figure 5 demonstrates average exposures for drug starting at 1st, 2nd, 3rd and after 3rd days since admission. It helps detect drugs of suspecious high exposures.
...
final_out<-list(Table1=consort_tbl,
                Table2=tbl1_summ,
                Table3=tbl1_summ2,
                Table4=demo_nice_tbl,
                Table5=vital_summ,
                Table6=vital_smoke_summ,
                Table7=lab_summ,
                Table8=DRG_summ,
                Table9=dx_summ,
                Table10=px_summ,
