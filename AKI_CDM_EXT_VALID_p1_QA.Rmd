...
kable(lab_summ %>% filter(freq_rk <=20), 
      caption="Table 3 - Lab Summaries (top20)")

lab_temp<-lab_summ %>%
  filter(summ %in% c("1.encounters#",
                     "2.records#",
                     "3e.cov")) %>%
  dplyr::select(key,summ,overall) %>% unique %>%
  mutate(overall=ifelse(overall=="<11",10,as.numeric(overall))) %>%
  spread(summ,overall) %>%
  mutate(record_per_enc=round(`2.records#`/`1.encounters#`)) %>%
  arrange(desc(`1.encounters#`))

lab_top<-lab_temp %>% slice(1:20)

ggplot(lab_temp,aes(x=`3e.cov`,y=`1.encounters#`,color=record_per_enc)) +
  geom_point()+scale_color_gradient2(low="grey",mid="blue",high="red")+
  scale_y_continuous(sec.axis = sec_axis(trans= ~./enc_tot,
                                         name = 'Percentage'))+
  geom_text(lab_top,aes(label=key),hjust=0,vjust=0)+
  labs(x="COV",y="Encounter Counts",
       title="Figure 2 - Data Density vs. Variability for Labs")
```
...
kable(DRG_summ %>% arrange(desc(`2.encounters#`)) %>% slice(1:20),
      caption="Table 4 - UHC DRG Summaries (top20)")

drg_temp<-DRG_summ %>% 
  dplyr::select(key1,key2,`2.encounters#`,`4c.mean_history`) %>%
  mutate(`4c.mean_history`=="<11",10,as.numeric(`4c.mean_history`)) %>%
  mutate(`2.encounters#`=as.numeric(`2.encounters#`),
         `4c.mean_history`=as.numeric(`4c.mean_history`))

drg_top<-drg_temp %>% group_by(key1) %>% slice(1:10) %>% ungroup

ggplot(drg_temp,aes(x=`4c.mean_history`,y=`2.encounters#`,color=key1)) +
  geom_point() + geom_text(drg_top,aes(label=key2),hjust=0,vjust=0)+
...
kable(dx_summ %>% dplyr::arrange(desc(`2.encounters#`)) %>% slice(1:20), 
      caption="Table 5 - Historical Diagnoses Summaries (top20)")
...
dx_top<-dx_temp %>% slice(1:20) %>% ungroup

ggplot(dx_temp,aes(x=`4c.mean_history`,y=`2.encounters#`)) +
  geom_point() + geom_text(dx_top,aes(label=key),hjust=0,vjust=0)+
...
kable(px_summ %>% arrange(desc(`2.encounters#`)) %>% slice(1:20),
      caption="Table 6 - Historical Procedures Summaries (top20)")
...
px_top<-px_temp %>% slice(1:20) %>% ungroup

ggplot(px_temp,aes(x=`4c.mean_history`,y=`2.encounters#`)) +
  geom_point() + geom_text(px_top,aes(label=key),hjust=0,vjust=0)+
...
med_top<-med_temp %>% slice(1:20)

ggplot(med_temp,aes(x=dsa_grp,y=`3b.median_expos`,color=`1.encounters#`)) +
  geom_point() + scale_color_gradient2(low="grey",mid="blue",high="red")+
  geom_text(med_top,aes(label=key),hjust=0,vjust=0)+
